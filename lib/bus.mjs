...
// Render a message as a one-line peer notice. The from-label is suffixed with a short
// slice of the origin (the session id) so that sessions sharing a branch name stay
// distinguishable in the feed. Shared by the inbox tool and the receive hook.
export function formatMessage(m) {
  const when = new Date(m.ts).toISOString().slice(11, 16); // HH:MM UTC
  const who = m.origin ? `${m.from}#${String(m.origin).slice(0, 8)}` : m.from;
