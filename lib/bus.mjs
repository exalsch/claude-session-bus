...
// Per-session origin. The MCP server and the hook are both spawned by the same
// Claude Code process, so process.ppid is identical within a session and differs
// across sessions. This is the key that lets the receiver filter out its own
// broadcasts. (See spec "Risks" - validate this holds on Windows in the spike.)
